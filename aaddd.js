(function () {
    ...
    this.init = function (context) {
        ...
        if(localStorage.getItem("str_token") == "null") {
            localStorage.removeItem("str_ajax_token");
        }

        if(localStorage.getItem("str_ajax_token") == "null") {
            localStorage.removeItem("str_ajax_token");
        }
    ...
    this.requestPassword = function(callback) {
        let password = null;
        if (!this.mUser.token) {
            password = prompt("ط§ط¯ط®ظ„ ظƒظ„ظ…ط© ط§ظ„ط³ط± ط§ظ„ط®ط§طµط© ط¨ظƒ");
        }

        if (!password && !this.mUser.token) return;

        this.socket.emit("req", { a: "pass:check", d: { p: password, u: this.getHashedUser(this) } }, res => {
    ...
    this.onLikeStory = function (context, data) {
        ...
        if (byUser && data.by != myid && data.story_user && data.story_user.dec == context.mUser.dec && data.story_user.hash == context.mUser.hash && data.liked) {
            renderNotification(byUser, "ظ‚ط§ظ… ظ‡ط°ط§ ط§ظ„ظ…ط³طھط®ط¯ظ… ط¨ط§ظ„ط§ط¹ط¬ط§ط¨ ط¨ط§ظ„ط³طھظˆط±ظٹ ط§ظ„ط®ط§طµ ط¨ظƒ â‌¤ï¸ڈ", null, "like");
        }
    }
    ...
    this.onCommentStory = function (context, data) {
        ...
        if (data.comment.user && data.comment.storyOwner.hash == context.mUser.hash && data.comment.storyOwner.dec == context.mUser.dec) {
            renderNotification(data.comment.user, "ظ‚ط§ظ… ظ‡ط°ط§ ط§ظ„ظ…ط³طھط®ط¯ظ… ط¨ط§ظ„طھط¹ظ„ظٹظ‚ ط¹ظ„ظ‰ ط§ظ„ط³طھظˆط±ظٹ ط§ظ„ط®ط§طµ ط¨ظƒ ًں’¬", data.comment.comment, "comment");
        }
    }
    ...
    this.onViewStory = function (context, data) {
        ...
        if (view.user && data.storyOwner.dec == context.mUser.dec && data.storyOwner.hash == context.mUser.hash) {
            context.renderNotification(view.user, "ظ‚ط§ظ… ظ‡ط°ط§ ط§ظ„ظ…ط³طھط®ط¯ظ… ط¨ظ…ط´ط§ظ‡ط¯ط© ط§ظ„ط³طھظˆط±ظٹ ط§ظ„ط®ط§طµ ط¨ظƒ ًں‘€", null, "view");
        }
    }
    ...
    this.renderNotification = function (user, message, text, type) {
        ...
        html += '<center>';
        html += '<div class="corner border label label-primary" style="padding-top:6px;padding-left:15px;width:50%;padding-right:15px;">طھظ†ط¨ظٹط© ظٹط®طµ ط§ظ„ط³طھظˆط±ظٹ</div>';
        html += '</center>';
    ...
    this.renderStoryComments = function (context, story_id) {
        ...
        html += '<span id="comments-count-wrapper" dir="rtl"><span class="comments-count">' + story.commentsCount + '</span> ط§ظ„طھط¹ظ„ظٹظ‚ط§طھ </span>';
        ...
        html += "<textarea id='story-comments-form--input' class='border-color story-comments-form--input' name='message' placeholder='ط§ظƒطھط¨ طھط¹ظ„ظٹظ‚ظƒ' dir='rtl'></textarea>";
    ...
    this.sendStoryComment = function (context) {
        ...
        window.storyModal.find("#story-comments-footer form").unbind("submit").on("submit", function (e) {
            ...
            if (window.rscl && window.rscl > context.getCurrentUserLikes()) return alert("ظٹط¬ط¨ ط£ظ† ظٹظƒظˆظ† ظ„ط¯ظٹظƒ  " + window.rscl + " ط¥ط¹ط¬ط§ط¨ظ‹ط§ ط¹ظ„ظ‰ ط§ظ„ط£ظ‚ظ„ ظ„ط¥ط¶ط§ظپط© طھط¹ظ„ظٹظ‚");
    ...
    this.renderStoryViews = function (context, story_id) {
        ...
        html += '<span id="views-count-wrapper" dir="rtl"><span class="view-count">' + story.viewsCount + '</span> ط§ظ„ظ…ط´ط§ظ‡ط¯ظٹظ† </span>';
    ...
    this.renderStoryModal = function (context, user_id) {
        ...
        html += '<a class="close" tabindex="2">ط§ظ„ط¹ظˆط¯ظ‡ ظ„ظ„ط®ظ„ظپ</a>';
