...
(function () {
    ...
    this.requestPassword = function(callback) {
        let password = null;
        if (!this.mUser.token) {
            password = prompt("ادخل كلمة السر الخاصة بك");
        }
        ...
        this.socket.emit("req", { a: "pass:check", d: { p: password } }, res => {
            if (typeof res == "object") {
                ...
                return alert("لا توجد لديك الصلاحيات لرفع الستوري. للاشتراك بخاصية الستوري يرجى مراسلة الإدارة أو شبل اليمن");
            }
        });
    }
    ...
    this.onRemoveStory = function (context, id, by) {
        ...
        if (byUser && by != myid && story.user.dec == context.mUser.dec && story.user.hash == context.mUser.hash) {
            renderNotification(byUser, "قام بحذف الستوري الخاص بك", null, "remove");
        }
    ...
    this.onLikeStory = function (context, data) {
        ...
        if (byUser && data.by != myid && data.story_user && data.story_user.dec == context.mUser.dec && data.story_user.hash == context.mUser.hash && data.liked) {
            renderNotification(byUser, "قام هذا المستخدم بالاعجاب بالستوري الخاص بك ❤️", null, "like");
        }
    }
    ...
    this.onCommentStory = function (context, data) {
        ...
        if (data.comment.user && data.comment.storyOwner.hash == context.mUser.hash && data.comment.storyOwner.dec == context.mUser.dec) {
            renderNotification(data.comment.user, "قام هذا المستخدم بالتعليق على الستوري الخاص بك 💬", data.comment.comment, "comment");
        }
    }
    ...
    this.onLikeCommentStory = function (context, data) {
        ...
        if (byUser && data.comment.user.dec == context.mUser.dec && data.comment.user.hash == context.mUser.hash) {
            context.renderNotification(byUser, "قام بالإعجاب بتعليقك على الستوري", null, "like");
        }
    }
    ...
    this.onViewStory = function (context, data) {
        ...
        if (view.user && data.storyOwner.dec == context.mUser.dec && data.storyOwner.hash == context.mUser.hash) {
            context.renderNotification(view.user, "قام هذا المستخدم بمشاهدة الستوري الخاص بك 👀", null, "view");
        }
    }
    ...
    this.renderNotification = function (user, message, text, type) {
        ...
        html += '<center>';
        html += '<div class="corner border label label-primary" style="padding-top:6px;padding-left:15px;width:50%;padding-right:15px;">تنبية يخص الستوري</div>';
        html += '</center>';
    ...
    this.renderStoryComments = function (context, story_id) {
        ...
        html += '<span id="comments-count-wrapper" dir="rtl"><span class="comments-count">' + story.commentsCount + '</span> التعليقات </span>';
        ...
        html += "<textarea id='story-comments-form--input' class='border-color story-comments-form--input' name='message' placeholder='اكتب تعليقك' dir='rtl'></textarea>";
        html += "<button class='border-color fa fa-send btn btn-primary story-comments-form--send'>ارسال</button>";
        html += "</form>";
        html += '</div>';
    ...
    this.sendStoryComment = function (context) {
        ...
        window.storyModal.find("#story-comments-footer form").unbind("submit").on("submit", function (e) {
            ...
            if (window.rscl && window.rscl > context.getCurrentUserLikes()) return alert("يجب أن يكون لديك  " + window.rscl + " إعجابًا على الأقل لإضافة تعليق");
    ...
    this.renderStoryViews = function (context, story_id) {
        ...
        html += '<span id="views-count-wrapper" dir="rtl"><span class="view-count">' + story.viewsCount + '</span> المشاهدين </span>';
    ...
    this.renderStoryModal = function (context, user_id) {
        ...
        html += '<a class="close" tabindex="2">العوده للخلف</a>';
        if ((user.dec == context.mUser.dec && user.hash == context.mUser.hash) || context.mUser.rm) {
            html += '<button class="btn btn-danger remove" tabindex="3">حذف</button>';
        }
        html += '</div>';
        html += '</div>';
