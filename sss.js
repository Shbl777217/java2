...
setInterval(renderStoryButton, 20000);
...
function renderStoryButton() {
    if(!$(users).hasClass("active")) return;
    te3b_story_users.forEach(function(user) {
        checkStoryRequest(md5(user.dec)).then(res => {
            res = JSON.parse(res);
            const uzr = $("#users .uzr:contains('"+user.dec+"')");
            uzr.css("position", "relative");
            if(!uzr || !uzr.length) return;
            if(res.error == false && res.file) {
                uzr.find('.str').unbind("click").remove();
                uzr.find('.uhash').hide();
                uzr.find('.co.ico').hide();
                uzr.append('<span onclick="renderStory(event);" data-file="'+res.file+'" style="box-shadow: 0 0 3px 0 #ccc; cursor: pointer; text-align: center; display: inline-block; width: 20px; height: 20px; border-radius: 50%;font-size: 4rem; position: absolute; right: 5px; top: 2px; color: white; background-color: red" class="str fa fa-user"></span>');
            } else {
                uzr.find('.uhash').show();
...
function renderStory(e) {
    ...
    const file = "https://te3b.net/uploads/stories/"+e.target.dataset.file;
    $(body).append('<div class="str-container" style="position: absolute; background: #000000BB; display: flex; align-items: center; justify-content: center; margin: 0; padding: 0; top: 0; bottom: 0; right: 0; left: 0;"> <video onclick="videoStoryClicked(this);" style="width: auto; max-height: 80%; border-radius: 10px; box-shadow: 0 0 25px 0 #333;" src="'+file+'" autoplay loop></video><span onclick="removeStory();" style="position: absolute; top: 5px; right: 5px; font-size: 1rem; border-radius: 50%" class="btn btn-danger">X</span></div>');
}
...
function setStory(input) {
    ...
    makeRequest('POST', 'https://www.te3b.net/story', formData, input, loader, button, function (err, response) {
...
function checkStoryRequest(name) {
    return new Promise((resolve, reject) => {
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "https://www.te3b.net/story/"+name);
        xhr.onload = function () {
